...
mtcars2.head()
#Method3 - excel
mtcars3 = pd.read_excel('mtcarsExcel.xlsx',0)
mtcars3.head()
...
mtcars.shape
len(mtcars)                                # print number of rows

len(mtcars.columns)                        # number of columns
print("{} Rows x {} Columns ".format(mtcars.shape[0],mtcars.shape[1]))
len(list(mtcars))

mtcars.columns                              # print names of columns 
...
len(mtcars.loc[mtcars['am'] == 1])

#First Row
mtcars.head(2)
# Acces particular location
mtcars.iloc[0:len(mtcars),0:5]
...
pd.set_option('display.max_columns', len(mtcars.columns))
# mpg > 23 or wt < 2 ; Conditional filtering
...
mtcars.loc[lambda x: x['mpg'] > 23].loc[lambda x: x['wt'] < 2].loc[lambda y: y['carb'] == 2]
